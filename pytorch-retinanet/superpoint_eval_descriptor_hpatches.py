        ...
        for pair_name in pairs_name:
            ...
            for idx, data in enumerate(dataloader_val):
                with torch.no_grad():
                    ...
                    #img_gray = np.array(img_gray.resize(size=(240, 320)))
                    ...
                    pts, desc ,heatmap = post_processing_superpoint_detector(F.softmax(output_semi.squeeze() ,dim=1),output_desc , H, W, conf_thresh=t_student, border_remove=5)
                    pts_student.append(pts)
                    heatmap_student.append(heatmap)
                    desc_student.append(desc)
                    """
                    ...
                    pts, desc, heatmap  = post_processing_superpoint_detector(out_dect_teacher, out_desc_teacher ,H, W, conf_thresh=t_teacher, border_remove=5)
                    pts_teacher.append(pts)
                    heatmap_teacher.append(heatmap)
                    desc_teacher.append(desc)

            data_keypoints_teacher = {}
            data_keypoints_teacher['prob'] = heatmap_teacher[0][:-32,:-32]
            data_keypoints_teacher['desc'] = desc_teacher[0][:-32,:-32]
            ...
            if len(heatmap_teacher) == 1:
                ...
                data_keypoints_teacher['warped_desc'] = desc_teacher[0][:-32,:-32]
                data_keypoints_teacher['homography'] = np.identity(3)
                imgs_gray.append(img_gray[:-32,:-32])
                pts_teacher.append(pts)
            else:
                data_keypoints_teacher['warped_prob'] = heatmap_teacher[1][:-32,:-32]
                data_keypoints_teacher['warped_desc'] = desc_teacher[1][:-32,:-32]
            ...
            data_keypoints_student['desc'] = desc_student[0][:-32,:-32]
            ...
            if len(heatmap_student) == 1:
                ...
                data_keypoints_student['warped_desc'] = desc_student[0][:-32,:-32]
                ...
                data_keypoints_student['warped_desc'] = desc_student[1][:-32,:-32]
            """
            ...
            #save_path = '/home/baudoin/pytorch-retinanet-pipeline/pytorch-retinanet/output_images/img_'+ str(i_img) + '.jpeg'
            #plot_images(imgs_gray[0], imgs_gray[1], pts_teacher[0].T, pts_teacher[1].T, save_path)                    
        print(homography_estimation(data_keypoints_teacher_paths, keep_k_points=1000, correctness_thresh=3, orb=False))
        quit()
...
def post_processing_superpoint_detector(semi, coarse_desc, H, W ,conf_thresh=0.015, cell=8, border_remove=8, nms_dist=3):
    ...
    if pts.shape[1] == 0:
        ...
        samp_pts = torch.from_numpy(np.indices((H,W)).reshape(2,-1))
        #samp_pts = torch.tensor(np.indices((H, W)).T.reshape(-1,2)).unsqueeze(0).unsqueeze(0)
...
def plot_images(image, warped_image, pts, pts_warped, save_path):
    ...
    img = warped_image
    pts = pts_warped#data['warped_prob'][:,[1,0,2]]
